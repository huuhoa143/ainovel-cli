...
func renderWelcome(width, height int, errMsg string, mode startupMode, importHint string) string {
	...
	features := []struct{ icon, label, desc string }{
		{">>", i18n.F("多模型协作"), i18n.F("Architect 规划 / Writer 创作 / Editor 审阅")},
		{"::", i18n.F("断点恢复"), i18n.F("崩溃或中断后从上次进度自动续写")},
		{"<>", i18n.F("实时干预"), i18n.F("创作过程中随时调整剧情走向")},
		{"##", i18n.F("分层长篇"), i18n.F("支持卷-弧-章分层结构的长篇创作")},
	}
	...
	// 输入提示
	prompt := lipgloss.NewStyle().Foreground(bodyTextColor).Render(i18n.F("在下方输入你的小说需求开始创作"))

	modeLine := lipgloss.NewStyle().
		Foreground(colorMuted).
		Render(i18n.F("当前模式：") + mode.label() + " · " + mode.subtitle())

	// 示例
	examples := []string{
		i18n.F("写一部 12 章都市悬疑小说，主角是一名女法医"),
		i18n.F("创作一部仙侠长篇，主角从凡人修炼至飞升"),
		i18n.F("写一个科幻短篇，讲述 AI 觉醒后的伦理困境"),
	}
	...
	if importHint != "" {
		...
		b.WriteString(lipgloss.NewStyle().Foreground(colorDim).
			Render(i18n.F("已有小说存稿想接着写？输入 /import <文件路径> 导入后续写")))
	}
	b.WriteString("\n\n")
	b.WriteString(lipgloss.NewStyle().Foreground(colorDim).Italic(true).
		Render(i18n.F("Tab 切换模式 · 快速开始下 Enter 直接创作 · 共创规划下 Enter 进入对话")))
