...
func (t *SaveArcSummaryTool) Description() string {
	return i18n.F("保存弧级摘要和角色状态快照（长篇模式，弧结束时调用）")
}
func (t *SaveArcSummaryTool) Label() string { return i18n.F("保存弧摘要") }
...
func (t *SaveArcSummaryTool) Schema() map[string]any {
	snapshotSchema := schema.Object(
		...
		schema.Property("status", schema.String(i18n.F("当前状态（存活/受伤/失踪等）"))).Required(),
		schema.Property("power", schema.String(i18n.F("能力变化"))),
		schema.Property("motivation", schema.String(i18n.F("当前动机"))).Required(),
		schema.Property("relations", schema.String(i18n.F("关键关系变化"))),
	)
	voiceSchema := schema.Object(
		schema.Property("name", schema.String(i18n.F("角色名"))).Required(),
		schema.Property("rules", schema.Array(i18n.F("2-3 条语言特征规则（每条 ≤30 字）"), schema.String(""))).Required(),
	)
	styleRulesSchema := schema.Object(
		schema.Property("prose", schema.Array(i18n.F("3-5 条叙述风格规则（每条 ≤50 字，要具体可执行）"), schema.String(""))).Required(),
		schema.Property("dialogue", schema.Array(i18n.F("核心角色的对话特征规则"), voiceSchema)).Required(),
		schema.Property("taboos", schema.Array(i18n.F("本小说需避免的写法"), schema.String(""))),
	)
	return schema.Object(
		schema.Property("volume", schema.Int(i18n.F("卷号"))).Required(),
		schema.Property("arc", schema.Int(i18n.F("弧号"))).Required(),
		schema.Property("title", schema.String(i18n.F("弧标题"))).Required(),
		schema.Property("summary", schema.String(i18n.F("弧摘要（500字以内）"))).Required(),
		schema.Property("key_events", schema.Array(i18n.F("弧内关键事件"), schema.String(""))).Required(),
		schema.Property("character_snapshots", schema.Array(i18n.F("角色状态快照"), snapshotSchema)).Required(),
