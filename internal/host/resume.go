...
func describeArcEndLabel(store *storepkg.Store, progress *domain.Progress) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf(i18n.F("检查弧边界: %w"), err)
	}
	...
	if err != nil {
		return "", fmt.Errorf(i18n.F("读取弧评审: %w"), err)
	}
	hasArcSummary, err := store.Summaries.HasArcSummary(vol, arc)
	if err != nil {
		return "", fmt.Errorf(i18n.F("读取弧摘要: %w"), err)
	}
	hasVolumeSummary := false
	if boundary.IsVolumeEnd {
		hasVolumeSummary, err = store.Summaries.HasVolumeSummary(vol)
		if err != nil {
			return "", fmt.Errorf(i18n.F("读取卷摘要: %w"), err)
		}
	}
	switch {
	case !hasArcReview:
		return fmt.Sprintf(i18n.F("恢复：弧末评审待处理（V%d A%d）"), vol, arc), nil
	case !hasArcSummary:
		return fmt.Sprintf(i18n.F("恢复：弧摘要待生成（V%d A%d）"), vol, arc), nil
	case boundary.IsVolumeEnd && !hasVolumeSummary:
		return fmt.Sprintf(i18n.F("恢复：卷摘要待生成（V%d）"), vol), nil
	case boundary.NeedsExpansion && boundary.NextArc > 0:
		return fmt.Sprintf(i18n.F("恢复：待展开下一弧（V%d A%d）"), boundary.NextVolume, boundary.NextArc), nil
	case boundary.NeedsNewVolume:
		return fmt.Sprintf(i18n.F("恢复：待决策下一卷（V%d 末）"), vol), nil
	}
	return "", nil
}
